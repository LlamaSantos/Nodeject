(function (module) {
  ...
  var Nodeject = function Nodeject(options) {
    this.config = options || {};
    this.config.isCompoundKeys = this.config.compoundKeys && this.config.compoundKeys.delimiter;
    this.cache = {};
    this.categories = {};
  ...
  Nodeject.prototype.define = function (options) {
    ...
    var name = (this.config.isCompoundKeys && options.category ? (options.category + this.config.compoundKeys.delimiter) : '')
      + options.name;
    ...
    if (!(name in this.cache)) {
      if ('wrap' in options) {
        ...
        this.cache[name] = {
          ...
          fn: factory.call(options, options.type, toApply),
          deps: options.deps || [],
          instance: null,
        ...
        if (options.category) {
          ...
          if (isArray(arr)) {
            for (var i = 0; i < arr.length; i = i + 1) {
              addCategory(this.categories, arr[i], name);
            }
          } else {
            addCategory(this.categories, arr, name);
          }
        }
      }
    } else {
  ...
  Nodeject.prototype.resolve = function (options) {
    ...
    if (isString(options)) {
      ...
      name = (this.config.isCompoundKeys && options.category ? (options.category + this.config.compoundKeys.delimiter) : '')
        + options.name;
    } else if (options.category && isString(options.category)) {
      category = options.category;
    } else {
